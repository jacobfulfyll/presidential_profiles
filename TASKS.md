...
- [ ] paradox-table-unguarded-arithmetic: `convergence.py`'s `paradox_table` holds the module's only unguarded arithmetic — `others_mean = (d.sum() - d) / (n_all - 1)` divides by zero at `n_all == 1`, and `others_sd` (`ddof=1` over one element) is NaN at `n_all == 2`. Both emit a bare `RuntimeWarning` and write NaN into a **published** column (`z_vs_others`, the H2b effect size). Inert on real data (45 presidents) and flagged independently by QUALITY-CHECK and SIMPLIFY, but it is the one place the module degrades quietly rather than loudly. (QUALITY-CHECK + SIMPLIFY, convergence-analysis, 2026-07-21)
- [ ] selftest-leg-c-has-no-validity-reporting: `convergence.py`'s `_selftest` leg (c) builds a `draws` array that feeds the **published** `selftest.c_permutation_size` in `convergence_meta.json`, but unlike `permutation_null` it has no `n_valid` reporting and no trust gate — so a partially-evaporated selftest null would be invisible in the artifact. The array is now NaN-filled (so evaporation would show as NaN rather than recycled memory), but nothing counts or publishes how many draws survived. Same treatment `null_rows`/`null_status` already give the production null. (SIMPLIFY, convergence-analysis, 2026-07-21)
...
- [ ] bands-suite-now-coupled-to-build-state: an end-to-end test compares committed `docs/` HTML against `data/bands.parquet`, so a correct source change with a stale `docs/` fails the suite for a reason unrelated to the change — easy to misread as a regression. Intended coupling (it is what makes the check meaningful), but the failure message should say "rebuild the site" explicitly. (SIMPLIFY, suppress-degenerate-band-intervals, 2026-07-21)
- [ ] resolvable-floor-constant-vs-ci-low-can-disagree-under-patch: `bands.MIN_CLUSTERS_FOR_RESOLVABLE_CI` is bound at import while `write_bands` reads `CI_LOW` live. In production they cannot disagree (same source constant), but a monkeypatch of `CI_LOW` alone makes the meta publish an internally-consistent `floor_derivation` block beside a `min_clusters_for_resolvable_ci` field reporting a different operative floor. No test asserts they agree at write time. (SIMPLIFY, suppress-degenerate-band-intervals, 2026-07-21)
- [ ] cliponaxis-justification-rests-on-an-unasserted-property: `unresolved_traces`' `cliponaxis=False` is justified by "every flagged cell has `point == 0`", an empirical property of the current corpus that is documented but not asserted at that site. A future corpus with a flagged nonzero cell would leave the reasoning stale without failing anything. (SIMPLIFY, suppress-degenerate-band-intervals, 2026-07-21)
- [ ] issue-pages-rebuild-on-any-hover-format-change: every `docs/issues/*.html` inlines its full plotly figure JSON, so a one-character change to a hover format string rebuilds all 16 pages. Makes reviewing the genuinely-changed pages harder than it needs to be. (IMPLEMENT, suppress-degenerate-band-intervals, 2026-07-21)
- [ ] flagged-cells-outnumber-rendered-rings: 12 cells carry `interval_unresolvable` but only 11 rings render, because `topic_quality.display_issues` excludes `Discovered 4`. Pre-existing and correct, but any future prose claiming "N cells flagged" must not be read as "N markers on the site" — the two counts legitimately differ. (IMPLEMENT, suppress-degenerate-band-intervals, 2026-07-21)
