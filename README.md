...
[Compare workspace](docs/compare.html) overlays two or three profiles on one tab-switched corpus/AI
percentile radar stage, keeps exact measures in semantic table disclosures, and turns all five neighbor
instruments plus the supporting evidence into compact visual lanes, bars, portraits, and chips.
Agenda lenses remain distinct, downloads still fetch only the selected canonical profile shards,
and the charts use the bundled local Plotly runtime. The generated [Issues directory](docs/issues/index.html)
